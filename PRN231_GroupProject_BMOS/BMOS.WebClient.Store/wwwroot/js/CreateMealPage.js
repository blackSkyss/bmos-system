...
function GetProducts(products) {
    ...
    products.forEach((product) => {
        ...
        var BtnSelect = `<button class="btn BtnSelect" id="BtnSelect-${product.ID}" onclick="AddProductOfMeal(${product.ID})" disabled="disabled">disabled</button>`;
        if (product.Status == 1) {
            status = "Stocking";
...
function CreateMealForm() {
    ...
    files.forEach((image) => {
        formData.append("MealFileImages", image);
    });
    ...
    request.done((data, textStatus, jqXHR) => {
        $(".loading--part").css("display", "none");
        sessionStorage.setItem("CreateMealSuccessfully", "Create Meal Successfully.");
